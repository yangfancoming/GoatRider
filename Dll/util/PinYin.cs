...
namespace GoatTools
{
    public class PinYin     //string test = new PinYin().GetABC("山羊来了啊！");//ShanYangLaiLiaoA！  头文件添加引用using MyStrTools;
    {
        #region
        private static int[] getValue = new int[] //定义拼音区编码数组
            {
        ...
        private static string[] getStr = new string[]//定义拼音数组
            {
        ...
        public string GetABC(string str)//把汉字转换成首字母大写形式的拼音字母 
        {
        ...
        public string GetUpper(string str) //"Shan Yang Lai Le " == "SYLL"  获取拼音中的大写字母
        {
            string ends = "";
            char[] fuck = new char[str.Length];
            fuck = str.ToCharArray();//把string字符串转换成char字符数组
            for (int i = 0; i < fuck.Length; i++)
            {
                if (fuck[i] >= 'A' && fuck[i] <= 'Z')
                {
                    ends = ends + fuck[i];
                }
            }
        ...
        /* exe调用示例
            string test = new PinYin().GetABC("山羊来了啊！");//using MyStrTools;   ShanYangLaiLiaoA！
            string test2 = new PinYin().GetUpper(test);        //SYLLA
